  ...
  async () => {
    ...
    while (true) {
      const [isSingleReady, isClusterReady] = await Promise.all([
        isContainerReady('metronomic-kurrentdb_promise_test_single.kurrentdb', `InaugurationManager in state (Leader`),
        isContainerReady('metronomic-kurrentdb_promise_test_cluster_node1.kurrentdb', '<LIVE> [Leader')
      ]);
