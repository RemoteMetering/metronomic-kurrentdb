...
describe(
  'HTTP Client - Persistent Subscription',
  () => {
    it('Should get and ack first batch of events written to a stream', async () => {
    ...
    it('Should ack and nack messages individually', async () => {
      const client = new KurrentDB.HTTPClient(getHttpConfig());
      const testStream = `TestStream-${generateEventId()}`;
    ...
    it('Should update persistent subscription ', async () => {
      const client = new KurrentDB.HTTPClient(getHttpConfig());
      const testStream = `TestStream-${generateEventId()}`;
    ...
    it('Should delete persistent subscription', (done) => {
      const client = new KurrentDB.HTTPClient(getHttpConfig());
      const testStream = `TestStream-${generateEventId()}`;
      ...
      client
        .writeEvents(testStream, events)
        .then(() =>
          client.persistentSubscriptions.assert(testSubscriptionName, testStream).then(() =>
            client.persistentSubscriptions.remove(testSubscriptionName, testStream).then(() =>
              client.persistentSubscriptions
                .getEvents(testSubscriptionName, testStream, 10)
                .then(() => {
    ...
    it('Should return persistent subscription info', async () => {
      const client = new KurrentDB.HTTPClient(getHttpConfig());
      const testStream = `TestStream-${generateEventId()}`;
    ...
    it('Should return persistent subscriptions info for a stream', async () => {
      const client = new KurrentDB.HTTPClient(getHttpConfig());
      const testStream = `TestStream-${generateEventId()}`;
    ...
    it('Should return persistent subscriptions info for all', async () => {
      const client = new KurrentDB.HTTPClient(getHttpConfig());
      const testStream = `TestStream-${generateEventId()}`;
