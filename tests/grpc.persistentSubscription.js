...
describe('gRPC Client - Persistent Subscription', () => {
  it(
    'Should get all events written to a persistent subscription stream after subscription is started',
    (done) => {
      ...
      client
        ...
        .then(() => {
          client.createPersistentSubscriptionToStream(testStream, groupName).then(() => {
            client
              .subscribeToPersistentSubscriptionToStream(testStream, groupName, onEventAppeared, onDropped)
              .then((subscription) =>
  ...
  it(
    'Subscription should fail when subscription does not exist yet',
    async () => {
      const client = new KurrentDB.GRPCClient(getGRPCConfig());
