...
describe('Http Client - Check Stream Exist', () => {
  ...
  it(
    'Should return rejected promise when the request error is anything other than a 404',
    (callback) => {
      ...
      client
        .checkStreamExists('Non_existentStream_wrong_port_config')
        .then(() => {
  ...
  it('Should throw an exception when timeout is reached', (callback) => {
    ...
    client
      ...
      .then(() =>
        client.checkStreamExists(testStream).then(() => {
