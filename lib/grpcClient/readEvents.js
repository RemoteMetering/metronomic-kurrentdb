...
export default (config, direction) => async (streamName, startPosition, count, resolveLinkTos) => {
  ...
  startPosition =
    startPosition === undefined && direction === 'backward' ? 'end' : isNaN(startPosition) ? 'start' : startPosition;
  count = count || 1000;
  ...
  try {
    ...
    for await (const event of connection.readStream(streamName, {
      ...
      maxCount: count,
      fromRevision: !isNaN(startPosition) ? BigInt(startPosition) : startPosition,
      resolveLinkTos
    })) {
      events.push(event);
    }
