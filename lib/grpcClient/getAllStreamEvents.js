...
import flattenArray from '../utilities/flattenArray.js';
import connectionManager from './connectionManager.js';
import mapEvents from './utilities/mapEvents.js';

const debug = debugModule('metronomic-kurrentdb:getAllStreamEvents');
...
export default (config) => async (streamName, chunkSize, startPosition, resolveLinkTos) => {
  ...
  async function getNextChunk(startPosition) {
    ...
    for await (const event of connection.readStream(streamName, {
      direction: 'forwards',
      maxCount: chunkSize,
      fromRevision: !Number.isNaN(startPosition) ? BigInt(startPosition) : startPosition,
      resolveLinkTos
    })) {
      events.push(event);
      hasNewEvents = true;
