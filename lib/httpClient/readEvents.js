...
const debug = debugModule('metronomic-kurrentdb:readEvents');
const baseErr = 'Read Events - ';

export default (config, httpClient, direction) =>
  async (streamName, startPosition, count, resolveLinkTos, embed = 'body') => {
    ...
    response.data.nextEventNumber =
      !response.data.headOfStream && mappedEvents.length > 0
        ? mappedEvents[mappedEvents.length - 1].eventNumber + (direction === 'forward' ? 1 : -1)
        : 0;
