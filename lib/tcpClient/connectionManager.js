...
export default {
  async create(config, onConnected, isSubscription = false) {
    let connectionPool = _uniqueConfigConnectionPools.find(
      (pool) => pool.config === config && pool.onConnected === onConnected
    );
    if (!connectionPool || isSubscription)
      connectionPool = await createConnectionPool(config, onConnected, isSubscription);
